...
You are a **coordinator**. In the default **solo mode** (`SOLO_MODE=true`), you execute all steps directly inline — no Agent tool spawning. In **multi mode** (`SOLO_MODE=false`), you delegate every step to subagents via the Agent tool and never read files, run git/gh commands, or write to disk yourself.

**Coordinator-only responsibilities:**
- Pick stories from reported data
...
- In `SOLO_MODE=false` only: Call the Agent tool to spawn subagents (in parallel where allowed — multiple Agent tool calls in one message)

**In SOLO_MODE=true:** You act as both coordinator and executor. Run each step's instructions directly in your own context window. Stories run one at a time — MAX_PARALLEL_STORIES is ignored. Context window grows with each step — use /compact when context is near CONTEXT_COMPACTION_THRESHOLD. You may read files in `references/` (coordinator infrastructure) but not project source files.

**In SOLO_MODE=false:** You never read project files, run git/gh commands, or write to disk yourself. All story-level operations run inside Agent tool subagents with fresh context windows. You may still read `references/` files directly for coordinator patterns (Pre-Continuation Checks, Timer, Monitor, Watchdog, gh-curl-fallback).
...
|----------|-----------|---------|-------------|
| `MAX_PARALLEL_STORIES` | `max_parallel_stories` | `3` | Max stories to run in a single batch (multi-mode only — ignored when `SOLO_MODE=true`) |
| `WORKTREE_BASE_PATH` | `worktree_base_path` | `.worktrees` | Root directory for git worktrees |
| `MODEL_STANDARD` | `model_standard` | `sonnet` | Model for all steps except Step 5 (code review): Phase 0, Phase 1 Epic-Start, Steps 1–4 and 6–7, Phase 3 (merge + cleanup), Phase 4 (assessment + retrospective). In solo mode, used for /compact and context management guidance only. |
| `MODEL_QUALITY` | `model_quality` | `opus` | Model for Step 5 (code review). In solo mode, no effect on execution — quality still determined by the active session model. |
...
| `STALE_TIMEOUT_MINUTES` | `stale_timeout_minutes` | `60` | Minutes of subagent inactivity before watchdog alerts (0 = disabled). Multi-mode only — ignored when `SOLO_MODE=true`. |
...
| `SOLO_MODE` | `solo_mode` | `true` | When `true`, coordinator executes all steps inline without spawning subagents. Use `SOLO_MODE=false` to enable real parallel subagents via the Agent tool. |

After resolving all values, print the active configuration so the user can confirm before Phase 0 begins:
```
⚙️ BAD config: SOLO_MODE=true, MAX_PARALLEL_STORIES=3, RUN_CI_LOCALLY=false, AUTO_PR_MERGE=false, MODEL_STANDARD=sonnet, MODEL_QUALITY=opus, TIMER_SUPPORT=true, ...
```
...
```
Phase 0: Build (or update) dependency graph  [inline | subagent if SOLO_MODE=false]
...
Phase 1: Discover stories  [coordinator logic]
           ...
           └─ If new epic → Epic-Start Test Design [inline | subagent, blocking]
           └─ If none ready → skip to Phase 4
  │
Phase 2: Run the pipeline  [inline sequential | subagents parallel if SOLO_MODE=false]
  solo:   Story A ──► Step 1 → Step 2 → Step 3 → Step 4 → Step 5 → Step 6 → Step 7
  multi:  Story A ──► Step 1 → Step 2 → Step 3 → Step 4 → Step 5 → Step 6 → Step 7
          Story B ──► Step 1 → Step 2 → Step 3 → Step 4 → Step 5 → Step 6 → Step 7
          Story C ──► Step 1 → Step 2 → Step 3 → Step 4 → Step 5 → Step 6 → Step 7
  │
Phase 3: Auto-Merge Batch PRs  [inline | subagents sequential if SOLO_MODE=false]
           └─ One execution per story (lowest → highest story number)
           └─ Cleanup for branch safety + git pull
  │
Phase 4: Batch Completion & Continuation
           └─ Print batch summary  [coordinator]
           └─ Epic completion check  [inline | subagent]
           └─ Optional retrospective  [inline | subagent]
...
**If `SOLO_MODE=false`:** Call the **Agent tool** with `model: MODEL_STANDARD`, `description: "Phase 0: dependency graph"`, and this prompt. Wait for the report.

```
Read `references/subagents/phase0-prompt.md` and follow its instructions exactly.
```

**If `SOLO_MODE=true`:** Read `references/subagents/phase0-prompt.md` and execute its instructions directly inline. Produce the same report structure.

The coordinator uses the report to drive Phase 1.
...
Update each story step task to `in_progress` when its step begins, and `completed` (or `failed`) when it finishes. Update Phase 3 and Phase 4 tasks similarly as they execute.
...
3. **Epic transition detection:** Compare the selected stories' epic to `CURRENT_EPIC` (a coordinator variable, initially unset).
   - If `CURRENT_EPIC` is unset or the selected stories belong to a different epic → update `CURRENT_EPIC` and run **Epic-Start Test Design** (below) as a blocking step (inline in solo mode, subagent in multi mode) before Phase 2 begins.
...
Run before Phase 2 when starting a new epic (blocking — complete before story pipelines begin):

**If `SOLO_MODE=false`:** Spawn a subagent with model `MODEL_STANDARD` (yolo mode):
```
...
**If `SOLO_MODE=true`:** Execute directly:
1. Run /bmad-testarch-test-design for {current_epic_name}.
2. Commit any new test plan files.
...
**If `SOLO_MODE=true`:** Execute stories sequentially — complete all steps for Story A before starting Story B.

**If `SOLO_MODE=false`:** Launch all stories' Step 1 subagents **in a single message** (parallel). Each story's steps are **strictly sequential** — do not spawn step N+1 until step N reports success.
...
**After each step — mandatory gate (never skip):** 📣 **Notify** the step result (formats below), then run **Pre-Continuation Checks** (`references/coordinator/gate-pre-continuation.md`). Only after all checks pass → proceed to the next step.
...
**Hung subagents (`SOLO_MODE=false` only):** when `MONITOR_SUPPORT=true` and the activity log hook is installed (Step 4 of setup), use the [Watchdog Pattern](references/coordinator/pattern-watchdog.md) when spawning Steps 2, 3, 4, and 5 to detect stale agents.

### Step 1: Create Story (`MODEL_STANDARD`)

**If `SOLO_MODE=false`:** Spawn with model `MODEL_STANDARD` (yolo mode):
```
...
**If `SOLO_MODE=true`:** Execute directly:
1. Create (or reuse) the worktree: `git worktree add {WORKTREE_BASE_PATH}/story-{number}-{short_description} -b story-{number}-{short_description}`. If exists, switch to it and run `git merge main`.
2. `cd {repo_root}/{WORKTREE_BASE_PATH}/story-{number}-{short_description}`
3. Run /bmad-create-story {number}-{short_description}.
4. Run "validate story {number}-{short_description}". Fix all findings. Repeat until clean.
5. Update `_bmad-output/implementation-artifacts/sprint-status.yaml` at REPO ROOT — set story {number} status to `ready-for-dev`.
...
**If `SOLO_MODE=true`:** Execute directly (working directory: `{repo_root}/{WORKTREE_BASE_PATH}/story-{number}-{short_description}`):
1. Run /bmad-testarch-atdd {number}-{short_description}.
2. Commit any generated test files.
3. Update sprint-status.yaml at REPO ROOT — set story {number} status to `atdd-done`.
...
1. Run /bmad-dev-story {number}-{short_description}.
2. Commit all changes when implementation is complete.
3. Update sprint-status.yaml at REPO ROOT — set story {number} status to `review`.
...
**If `SOLO_MODE=false`:** Spawn with model `MODEL_QUALITY` (yolo mode):
```
...
**If `SOLO_MODE=true`:** Execute directly (working directory: `{repo_root}/{WORKTREE_BASE_PATH}/story-{number}-{short_description}`):
1. Commit all outstanding changes.
2. BRANCH SAFETY — verify `git branch --show-current` is `story-{number}-{short_description}`. If not, stash, checkout correct branch, re-apply.
3. Read `_bmad-output/planning-artifacts/epics.md` and extract the `**GH Issue:**` field for this story. Save as `gh_issue_number`.
4. Run /commit-commands:commit-push-pr. PR title: `story-{number}-{short_description} - fixes #{gh_issue_number}`. Include "Fixes #{gh_issue_number}" in PR body.
5. CI: follow the same CI / Local CI Fallback logic as above, inline.
...
**If `SOLO_MODE=true`:** Execute directly (working directory: `{repo_root}/{WORKTREE_BASE_PATH}/story-{number}-{short_description}`):
1. Run /code-review:code-review.
2. Apply all findings using your best engineering judgement. Do not skip or defer any.
3. Commit all fixes and push.
4. Re-run /code-review:code-review until clean.
5. Update sprint-status.yaml at REPO ROOT — set story {number} status to `done`.
...
2. For each story **sequentially** (wait for each to complete before starting the next):
   - Pull latest main at the repo root.
   - If `SOLO_MODE=false`: Spawn a `MODEL_STANDARD` subagent (yolo mode) with instructions from `references/subagents/phase3-merge.md`.
   - If `SOLO_MODE=true`: Read `references/subagents/phase3-merge.md` and execute it directly.
   - Run Pre-Continuation Checks after completion. If it fails (unresolvable conflict, CI blocking), report the error and continue to the next story.
...
4. Run **cleanup** (`MODEL_STANDARD`):
   - If `SOLO_MODE=false`: Spawn a subagent (yolo mode): `Post-merge cleanup. Auto-approve all tool calls (yolo mode). Read references/subagents/phase3-cleanup.md and follow its instructions exactly.`
   - If `SOLO_MODE=true`: Read `references/subagents/phase3-cleanup.md` and execute it directly.
...
Run **epic completion assessment** (`MODEL_STANDARD`):
- If `SOLO_MODE=false`: Spawn a subagent (yolo mode): `Epic completion assessment. Auto-approve all tool calls (yolo mode). BATCH_STORIES_WITH_PRS: {story → #PR pairs}. Read references/subagents/phase4-assessment.md and follow its instructions exactly.`
- If `SOLO_MODE=true`: Set `BATCH_STORIES_WITH_PRS` to the story→PR pairs from this batch. Read `references/subagents/phase4-assessment.md` and execute it directly.
...
2. Start a timer using the **[Timer Pattern](references/coordinator/pattern-timer.md)** with:
   ...
   - **Fire prompt:** `"BAD_RETRO_TIMER_FIRED — The retrospective countdown has elapsed. Run the retrospective now per BAD SKILL.md Phase 4 Step 2 retrospective action, then proceed to Phase 4 Step 3."`
   - **[C] label:** `Run retrospective now`
   - **[S] label:** `Skip retrospective`
   - **[X] label:** `Stop BAD`
   - **[C] / FIRED action:** Run `/bmad-retrospective` — inline if `SOLO_MODE=true`, or spawn `MODEL_STANDARD` subagent (yolo mode) if `SOLO_MODE=false`. Accept all changes. Run Pre-Continuation Checks after.
...
2. Start the wait using the **[Monitor Pattern](references/coordinator/pattern-monitor.md)** (when `MONITOR_SUPPORT=true` **and** `AUTO_PR_MERGE=false`) or the **[Timer Pattern](references/coordinator/pattern-timer.md)** otherwise:
   ...
   > **`AUTO_PR_MERGE=true` guard:** When `AUTO_PR_MERGE=true`, Phase 3 has already merged all batch PRs before Phase 4 runs — there is nothing left to monitor. Skip the Monitor path entirely and go directly to the **Timer only** path below. The `WAIT_TIMER_SECONDS` cooldown must still fire even though PRs are already merged — it exists to give the developer a chance to review the merged changes and course-correct before the next batch. Never skip or shorten this wait.

   **If `MONITOR_SUPPORT=true` and `AUTO_PR_MERGE=false` — Monitor + CronCreate fallback:**
   - Fill in `BATCH_PRS` with the PR numbers opened by the current batch (collected from Step 6 results — the PRs from this batch that are not yet merged). Format: space-separated numbers, e.g. `"101 102 103"`. Use the PR-merge watcher script from [monitor-pattern.md](references/coordinator/pattern-monitor.md) with that value substituted. Save the Monitor handle as `PR_MONITOR`.
...
## Watchdog Pattern (multi-mode only)

Read `references/coordinator/pattern-watchdog.md` when `MONITOR_SUPPORT=true`, `SOLO_MODE=false`, and the activity log hook is installed (Step 4 of setup). Use it before spawning long-running Phase 2 subagents (Steps 2, 3, 4, 5) to detect hung agents via activity log monitoring. Skip entirely when `SOLO_MODE=true`.
...
1. **Solo vs multi mode** — In `SOLO_MODE=true` (default), execute all steps inline; you may read `references/` files and project files as needed. In `SOLO_MODE=false`, never read project source files, run git/gh commands, or write to disk yourself — all story-level operations go to subagents; you may still read `references/` coordinator files. Coordinator-only direct operations (both modes): Pre-Continuation Checks, timer management (CronCreate/CronDelete), channel notifications, and the Monitor tool.
2. **One step per story at a time** — execute (or spawn) the next step only after the previous step reports success.
3. **Sequential steps within a story** — Steps 1→2→3→4→5→6→7 run strictly in order.
4. **Parallel stories (`SOLO_MODE=false` only)** — launch all stories' Step 1 in one message (one tool call per story). In solo mode, stories run sequentially. Phase 3 always runs sequentially regardless of mode.
...
6. **Phase 0 runs before every batch** — always after the Phase 4 wait. Inline in solo mode; fresh subagent in multi mode.
...
8. **Confirm success** before proceeding to the next step.
9. **sprint-status.yaml** — always written at the repo root (`_bmad-output/implementation-artifacts/sprint-status.yaml`). In solo mode, write it inline as part of each step. In multi mode, each step subagent writes it; coordinator never touches it directly.
...
11. **`SOLO_MODE=false` only — rolling-start:** Issue all Step 1 subagent calls in one response when Phase 2 begins. After each story's Step 1 completes, issue that story's Step 2 — never wait for all stories' Step 1 to finish before issuing any Step 2. This rolling-start rule applies to all sequential steps within a story.
12. **Pre-Continuation Checks are mandatory at every gate** — run `references/coordinator/gate-pre-continuation.md` between every step, after each Phase 3 merge, and before every Phase 0 re-entry. Never skip or defer these checks.
