...
test.describe("Reinstall Account Preservation — API/Integration Tests (ATDD, RED PHASE)", () => {
  ...
  test.skip("[P0][AC-1] same email/password sign-in after reinstall returns profile data intact", async ({
    request,
  }) => {
  ...
  test.skip("[P0][AC-1] two sign-ins with same email/password return the same userId — no duplicate user created", async ({
    request,
  }) => {
    ...
    const secondHeader = {
      Cookie: `better-auth.session_token=${second.token}`,
    };
  ...
  test.skip("[P0][AC-2] getSubscription returns same tier and status after simulated reinstall", async ({
    request,
  }) => {
  ...
  test.skip("[P0][AC-2] subscription data returned with only a fresh session — no device cache needed", async ({
    request,
  }) => {
  ...
  test.skip("[P0][AC-3] signing in with Google using the same email as an existing email/password account returns the same userId", async ({
    request,
  }) => {
    ...
    const originalProfileRes = await request.get(
      "/api/rpc/rebuild.getProfile",
      {
        headers: emailAuth,
      }
    );
    ...
    const _googleLinkRes = await request.post("/api/auth/sign-in/social", {
      data: {
        provider: "google",
        callbackURL: "/",
      },
  ...
  test.skip("[P0][AC-3] email-based account linking does not add phone-number provider to trustedProviders", async ({
    request,
  }) => {
  ...
  test.skip("[P0][AC-4] linkAccount re-points phone account row to current Google userId and returns linked: true", async ({
    request,
  }) => {
  ...
  test.skip("[P0][AC-4] linkAccount returns NOT_FOUND when phone number has no matching account", async ({
    request,
  }) => {
    // THIS TEST WILL FAIL — T2 (linkAccount procedure) not yet implemented
  ...
  test.skip("[P1][AC-4] linkAccount returns same_account when phone number belongs to current user", async ({
    request,
  }) => {
    // THIS TEST WILL FAIL — T2 (linkAccount procedure) not yet implemented
  ...
  test.skip("[P1][AC-4] linkAccount returns CONFLICT when current user already has a phone-number sign-in method", async ({
    request,
  }) => {
    // THIS TEST WILL FAIL — T2 (linkAccount procedure) not yet implemented
  ...
  test.skip("[P1][AC-4] linkAccount requires authentication — unauthenticated request returns 401", async ({
    request,
  }) => {
    // THIS TEST WILL FAIL — T2 (linkAccount procedure) not yet implemented
  ...
  test.skip("[P1][AC-5] linkAccount preserves current userProfile when current user already has one", async ({
    request,
  }) => {
  ...
  test.skip("[P1][AC-5] linkAccount migrates userProfile from old phone account when current user has no profile", async ({
    request,
  }) => {
    // THIS TEST WILL FAIL — T2 (conditional profile migration) not yet implemented
  ...
  test.skip("[P0][AC-6] cefrLevel, nativeLanguage, totalCallCount accessible after re-authentication", async ({
    request,
  }) => {
  ...
  test.skip("[P0][AC-6] call history with ratings accessible via API after re-authentication without device data", async ({
    request,
  }) => {
