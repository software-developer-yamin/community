...
test.describe("Cancellation Preserves Access — E2E Tests (ATDD, RED PHASE)", () => {
  ...
  test.skip(
    "[P0][AC-1] subscription page shows 'Access until <date>' banner after cancellation",
    async ({ page }) => {
      ...
      await expect(
        page.getByText(/your access continues until/i)
      ).toBeVisible();
  ...
  test.skip(
    "[P0][AC-1] subscription page shows 'Cancelled' status badge after cancellation",
    async ({ page }) => {
      ...
      await expect(
        page.getByRole("button", { name: /re-enable auto-renew|reactivate/i })
      ).toBeVisible();
    }
  );
  ...
  test.skip(
    "[P1][AC-3] clicking 're-enable auto-renew' restores subscription and hides access-until banner",
    async ({ page }) => {
      ...
      // Click re-enable
      await page.getByRole("button", { name: /re-enable auto-renew|reactivate/i }).click();
  ...
  test.skip(
    "[P1][T5/T6] subscription page shows correct tier expiry date for cancelled user",
    async ({ page }) => {
  ...
  test.skip(
    "[P0][AC-6] gender preference settings are accessible for cancelled user still in paid window",
    async ({ page }) => {
  ...
  test.skip(
    "[P0][AC-5] gender preference settings are locked after tierExpiresAt has passed",
    async ({ page }) => {
      ...
      const genderPrefSelect = page.getByTestId("gender-preference-select");
      const upgradeBanner = page.getByText(/upgrade to premium/i);
